...
export function CompositionSVG({
  ...
  className?: string;
  /** `dark` matches the 3D paper backdrop, so the fallback is not a flash of white. */
  tone?: 'paper' | 'dark';
}) {
  const ground = tone === 'dark' ? '#08080a' : 'var(--color-paper)';
  const line = tone === 'dark' ? '#F2EDE1' : 'var(--color-ink)';
  return (
    <svg
      viewBox="0 0 400 300"
      className={className}
    ...
    >
      ...
      <rect x="0" y="252" width="400" height="8" fill={line} />
      <rect x="248" y="0" width="6" height="300" fill={line} />

      {/* Blue circle — Kandinsky's pairing. */}
      <circle cx="120" cy="150" r="74" fill="var(--bh-blue)" stroke={line} strokeWidth="5" />
      ...
      <path d="M252 300 L400 96" stroke={line} strokeWidth="5" fill="none" />

      {/* Small black square, off-grid, for balance. */}
      <rect x="316" y="36" width="34" height="34" fill={line} />
    </svg>
  );
}
