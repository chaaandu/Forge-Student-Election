import { z } from 'zod';
import { ELECTION_STATUSES, POSITION_KINDS, VOTER_TYPES, ZERO_TURNOUT_POLICIES, } from './types.js';
...
const photoUrl = z
    ...
    .min(1)
    .max(2048)
    .refine((v) => v.startsWith('/') || v.startsWith('https://'), 'must be a root-relative path ("/candidates/x.jpg") or an https URL');
const voterTypeSchema = z.enum(VOTER_TYPES);
const weightsSchema = z
    .object(Object.fromEntries(VOTER_TYPES.map((t) => [t, z.number().min(0).max(1)])))
    .refine((w) => Math.abs(VOTER_TYPES.reduce((s, t) => s + w[t], 0) - 1) <= WEIGHT_SUM_TOLERANCE, {
...
function crossValidate(config) {
    ...
    for (const position of config.positions) {
        ...
        if (position.kind === 'house-captain') {
            if (position.houseId === undefined)
                push(at, 'house-captain positions must set houseId');
            if (position.eligibility.houseId === undefined)
                push(at, 'house-captain positions must set eligibility.houseId, otherwise every student in ' +
        ...
        const weightSum = position.eligibility.voterTypes.reduce((sum, t) => sum + config.election.weights[t], 0);
        if (weightSum <= 0)
            push(at, `eligible voter types [${position.eligibility.voterTypes.join(', ')}] have a combined ` +
                `weight of 0, so this position could never be scored`);
    }
    for (const candidate of config.candidates) {
        if (!positionIds.has(candidate.positionId))
            push(`candidates.${candidate.id}`, `positionId "${candidate.positionId}" is not a declared position`);
    }
    return issues;
}
/**
 ...
 */
export function parseElectionConfig(input) {
    const parsed = rawConfigSchema.safeParse(input);
    if (!parsed.success)
        throw new ConfigValidationError(zodIssues(parsed.error));
 ...
 */
export function parseVoterRoll(input, config) {
    const parsed = z.array(voterSchema).safeParse(input);
    if (!parsed.success)
        throw new ConfigValidationError(zodIssues(parsed.error));
    const voters = parsed.data;
    const issues = [];
    const houseIds = new Set(config.houses.map((h) => h.id));
    ...
    for (const voter of voters) {
        ...
        if (voter.type === 'student' && hasHouseContests && voter.houseId === undefined)
            issues.push({
                path: at,
                message: 'student has no house, so they would silently lose their house captain vote. ' +
                    'Assign a house, or remove the house captain positions.',
            });
        const eligibleFor = config.positions.filter((p) => p.eligibility.voterTypes.includes(voter.type) &&
